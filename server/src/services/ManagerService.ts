...
export class ManagerService {
  ...
  private async buildRiskFlags(
    ...
    const hoursSummary = await this.buildRecentHoursSummary(projectId, allocations);
  ...
  private async buildProjectFacts(projectId: number): Promise<ProjectFacts> {
    ...
    const recentHoursSummary = await this.buildRecentHoursSummary(projectId, allocations);
  ...
  private async buildRecentHoursSummary(
    projectId: number,
    allocations: AllocationResponseDto[],
    ...
    return Promise.all(
      allocations.map(async (allocation) => {
        const detail = await this.timesheetService.getEmployeeWeekDetail(
